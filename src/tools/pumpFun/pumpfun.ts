...
export const pumpFunSchema = z.object({
...
const createLimitOrderImplementation = async (
  args: z.infer<typeof pumpFunSchema>,
  response_id: string
): Promise<ToolResult<'pumpfun_order'>> => {
  useChatMessageHandler.getState().setCurrentChatItem({
    content: {
      type: 'loader_message',
      text: `Pumpfun agent: Creating limit order to ${args.action.toLowerCase()} ${args.amount} ${args.token}...`,
      response_id: 'temp',
      sender: 'system',
    },
    id: 0,
...
export const limitOrder = registerTool({
  name: 'limitOrder',
  description:
    'Call this function to buy/sell pumpfun tokens. e.g. command: buy/sell "token-name" for x sol',
